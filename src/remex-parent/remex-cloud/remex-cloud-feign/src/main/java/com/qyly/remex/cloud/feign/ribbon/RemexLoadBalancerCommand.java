...
import com.qyly.remex.utils.SocketUtils;
...
/**
 * 重写com.netflix.loadbalancer.reactive.LoadBalancerCommand<T>
 * 
 * @see com.netflix.loadbalancer.reactive.LoadBalancerCommand<T>
 * 
 * @author Qiaoxin.Hong
 *
 * @param <T>
 */
public class RemexLoadBalancerCommand<T> {

	private static final Logger logger = LoggerFactory.getLogger(RemexLoadBalancerCommand.class);
    ...
    public static class Builder<T> {
        ...
        @SuppressWarnings({ "unchecked", "rawtypes" })
		public RemexLoadBalancerCommand<T> build() {
            ...
            return new RemexLoadBalancerCommand<T>(this);
        }
    }
	...
	private RemexLoadBalancerCommand(Builder<T> builder) {
    ...
    private Func2<Integer, Throwable, Boolean> retryPolicy(final int maxRetrys, final boolean same, ExecutionInfoContext context) {
        return new Func2<Integer, Throwable, Boolean>() {
            ...
            public Boolean call(Integer tryCount, Throwable e) {
                ...
                try {
                	//只处理重试其它服务的
                	if (!same && (e instanceof SocketTimeoutException || e instanceof ConnectException)) {
                		Server curServer = context.getServer();
                		
                		if (curServer != null) {
                			/*
                			 * 当出现SocketTimeoutException异常时，则对是否重试进行一次处理，
                			 * 出现异常只有连接失败和服务处理失败两种，目前只对连接失败进行重试
                			 */
                			boolean connectionRight = SocketUtils.isConnectable(curServer.getHost(), curServer.getPort());
                			if (!connectionRight) {
                				logger.info("Retry feign server [appName : {}] [failedUrl : {}]"
                						, curServer.getMetaInfo().getAppName(), curServer.getId());
                				return true;
                			}
                		}
                	}
					...
					logger.error("ribbon tms retry error", e2);
				}
        ...
        Observable<T> o = 
                ...
                .concatMap(new Func1<Server, Observable<T>>() {
                    ...
                    public Observable<T> call(Server server) {
                        ...
                        if (maxRetrysSame > 0) 
                            o = o.retry(retryPolicy(maxRetrysSame, true, context));
                        return o;
                    }
                });
            
        if (maxRetrysNext > 0 && server == null) 
            o = o.retry(retryPolicy(maxRetrysNext, false, context));
